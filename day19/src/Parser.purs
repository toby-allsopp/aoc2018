...
fail message = Parser (\s -> Left $ message <> ": remaining: " <> View.showView 80 s)
...
takeWhile :: forall c. Show c => (c -> Boolean) -> Parser c (Array c)
takeWhile p = do
    s <- peek
    case View.takeWhile p s of
...
literal s = do
    ...
    unless (viewEq token sv) $ fail $ "expected " <> View.showView 80 sv <> "; got " <> show token
    pure s
...
word = takeWhile (not <<< isSpace) <#> fromCharArray
...
separatedList :: forall c a b. Parser c b -> Parser c a -> Parser c (Array a)
