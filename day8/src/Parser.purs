...
import View (class View, AnyView)
import View as View

data Parser c a = Parser (forall v. View v c => v -> Either String { result:: a, remaining:: AnyView c })

runParser :: forall f c a. View f c => Parser c a -> f -> Either String a
...
literal :: forall g c. View g c => Eq c => Show c => g -> Parser c g
literal s = do
    token <- take (View.length s)
    unless (viewEq token s) $ fail $ "expected " <> View.showView s <> "; got " <> show token
    pure s
